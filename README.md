...
1. `cd my-api/`
2. `mvn clean install -DskipTests | tee 1.0.1_log.txt`
...
3. `cd target/`
4. `java -jar my-api-1.0.0.1591234567.jar | tee 1.0.2_log.txt` 
...
2. To prevent permission issues, i enabled executable permissions by running the this command: `chmod +x updatepom.sh`.

3. I entered the command `./updatepom.sh pom.xml | tee 1.1.1_log.txt`. `./updatepom.sh pom.xml` runs the script and applies the update to the `pom.xml` file. I saved the full output log of the script with the `| tee 1.1.1_log.txt` argument.
...
8. copy the first command: `aws ecr get-login-password --region YOUR_AWS_REGION | docker login --username AWS --password-stdin YOUR_AWS_ACCOUNT_ID.dkr.ecr.us-east-1.amazonaws.com` to retrieve an authentication token and authenticate your Docker client to your registry.
9. cd to the location of your pom.xml file and create two files for logging the full output of the build and run process respectively. `touch 1.2.1_log.txt` and `touch 1.2.2_log.txt`
10. start the build process `mvn clean install -DskipTests docker:push | tee 1.2.1_log.txt`
11. check for the new image in your local repository: `docker images | tee 1.2.2_log.txt`. also confirm the image is deployed to the assigned container registry. Verify in AWS ECR or dockerhub.io
11. start the container `docker run -d --name=my_container_name -p 8080:8080 the_full_image_name:tag `
...
* Run `docker-compose up -d --build` to create the EFK stack
* Confirm the status of the 4 containers whith `docker ps`.
* Confirm all services are running on the same custom-bridge network (logging-stack_correvate): `docker network inspect logging-stack_correvate | tee custombridgenetwork.txt` 
...
* Logs generated and saved to `1.3_logs.txt` using `docker-compose –no-ansi logs | tee 1.3_log.txt`.
...
![Click Here](my-api/Question 3.jpg)
