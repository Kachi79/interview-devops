...
1. I will **familiarise myself with the existing setup** to get a good understanding of the current architecture. If the existing architecture is robust enough with **high availability** and **security**, I will build the new setup in the **already existing network infrastructure**. So the final infrastructure will run in the `same VPC`. 

2. First I will set up the `AWS Elastic Search domain` to achieve a **centralized storage location** for the log data from both enviroments (qa and prod) in the already existing setup. To achieve `high availability`, this setup will be provisioned in **three availability zones**. To ensure network `security`, all services in this setup will be provisioned in the `private subnet` with a `loadbalancer` in  the `public subnet`, listening to incoming traffic. A `main account` will also be setup with `IAM roles` and `policy permissions` for **resources** and **users**. 

3. In the two environments `QA and PROD`, I will replace `cloud watch with Fluentbit` and move `Fluentbit and the other existing infrastructure` to the `private subnet`. A `load balancer` in the `public subnet` will be listening and `distributing traffic` to **all applications** in the `private subnet`. The `QA and PROD environmentS` will be able to get regular updates and upgrades via a `NAT gateway`, associated to the `private route-table`. 

4. Finally, the QA and PROD environments will have incoming traffic from the load balancer hitting the EC2 auto-scaled application which sends the traffic to the queueing service (RabbitMQ) running in Fargate. The various java microservice applications in Fargate will respond by pulling and consuming the messages in their respective RabbitMQ's.  
Since Fluentbit is now configured, it will collect logs from the different applications making up the distributed application, skynet-app and forward them to the AWS Elastic Search domain, allowing for centralized log management and analysis. AWS Elastic Search domain provides a kibana endpoint for log accessing, visualization and analysis.

5. The applications in all three environments will be deployed in multiple availability zones and will exist in the private subnet for security. Load balancers will be setup in the public subnet to distribute incoming traffic to the necessary applications. The NAT gateway will be associated with the private route table to allow the applications a secure access outside the VPC when updates and upgrades are required.   
